...
public class FloatWindowPermission {
        ...
        if (hasPermission()) {
            ...
            if (Miui.rom()) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    Miui.req(CommonAppContext.sInstance);
                }
            } else {
    ...
    private void req() {
        Intent intent = new Intent(CommonAppContext.sInstance, FloatPermissionActivity.class);
    ...
    private boolean hasPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            return Settings.canDrawOverlays(CommonAppContext.sInstance);
        } else {
    ...
    private boolean hasPermissionBelowMarshmallow() {
        try {
            AppOpsManager manager = (AppOpsManager) CommonAppContext.sInstance.getSystemService(Context.APP_OPS_SERVICE);
    ...
    private boolean hasPermissionForO(Context context) {
        try {
            WindowManager mgr = (WindowManager) CommonAppContext.sInstance.getSystemService(Context.WINDOW_SERVICE);
