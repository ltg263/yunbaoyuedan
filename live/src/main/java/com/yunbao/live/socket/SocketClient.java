            ...
            switch (map.getString("_method_")) {
                ...
                case Constants.SOCKET_SEND_MSG://文字消息，点亮，用户进房间，这种混乱的设计是因为服务器端逻辑就是这样设计的,客户端无法自行修改
                    ...
                    if ("2".equals(msgtype)) {//发言，点亮
                        ...
                        chatBean.setAnchorLevel(map.getIntValue("level_anchor"));
                        chatBean.setAge(map.getString("age"));
                        chatBean.setSex(map.getIntValue("sex"));
...
//                        chatBean.setGuardType(map.getIntValue("guard_type"));
                        ...
                        LiveUserGiftBean u =new LiveUserGiftBean();
                        ...
                        chatBean.setAge(u.getAge());
                        chatBean.setSex(u.getSex());
                        chatBean.setContent(u.getUserNiceName()+WordUtil.getString(R.string.live_enter_room));
                ...
                case Constants.SOCKET_SEND_GIFT://送礼物
                    JSONObject ctObj=map.getJSONObject("ct");
                    String uname=map.getString("uname");
                    String toName=ctObj.getString("to_username");
                    String giftName=ctObj.getString("giftname");
                    String giftNum=ctObj.getString("giftcount");
                    ...
                    receiveGiftBean.setUserNiceName(uname);
                    receiveGiftBean.setToname(toName);
                    receiveGiftBean.setGiftCount(Integer.parseInt(giftNum));

                    String content=WordUtil.getString(com.yunbao.chatroom.R.string.gift_tip,uname,toName,giftName,giftNum);

                    LiveChatBean chatBean = new LiveChatBean();
                    chatBean.setUserNiceName(uname);
                    chatBean.setId(map.getString("uid"));
                    chatBean.setContent(content);
                    chatBean.setToUserNiceName(toName);
        ...
        private void processLinkMicAnchor(JSONObject map) {
            ...
            switch (action) {
                case 1://收到其他主播连麦的邀请的回调
                    ...
                    u.setSex(map.getIntValue("sex"));
                    u.setLevel(map.getIntValue("level"));
                    u.setAnchorLevel(map.getIntValue("level_anchor"));
