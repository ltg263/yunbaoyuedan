...
public class ChatGiftDialogFragment extends AbsDialogFragment implements View.OnClickListener, OnItemClickListener<String>, ChatGiftPagerAdapter.ActionListener {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            ...
            mLiveName=bundle.getString(Constants.NICKNAME);
        }
        loadData();
    }
...
//                                u.setCoin(coin);
                            ...
                            ChatReceiveGiftBean giftBean=mChatGiftBean.valueToChatReceiveGiftBean();
                            giftBean.setTouid(mLiveUid);
                            giftBean.setToname(mLiveName);

                            int count=1;
                            try {
                                count= Integer.parseInt(mCount);
                                giftBean.setGiftCount(count);
                            }catch (Exception e){
                                e.printStackTrace();
                            }
