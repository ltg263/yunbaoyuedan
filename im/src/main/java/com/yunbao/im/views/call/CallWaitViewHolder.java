...
public class CallWaitViewHolder extends AbsViewHolder2 {
    ...
    private void initSoundPlayer() {
       if(mSmallSoundPlayer==null){
          mSmallSoundPlayer=new SmallSoundPlayer.SoundPlayerBuilder(mContext).setLifeCycleModel(LifeMode.SELF).build();
       }
    }
    ...
    private void playSound() {
        initSoundPlayer();
        mSmallSoundPlayer.start();
        SoundResource soundResource=new SoundResource(mContext.getResources(),"ring.mp3");
        PlayConfig playConfig=PlayConfig.defaultConfig(mContext);
        playConfig.setNumber(20);
        mSmallSoundPlayer.play(soundResource,playConfig);
    }
    ...
    public View.OnClickListener acceptClickLisnter= new View.OnClickListener(){
        ...
        public void onClick(View v) {
            if(absCallViewHolder!=null){
               absCallViewHolder.acceptAndBrocast(userBean.getId());
            }
        }
    };
    ...
    public void startTimeOutWatch(){
        disposable= Observable.timer(CallConfig.TIME_OUT_DURCATION, TimeUnit.SECONDS).observeOn(AndroidSchedulers.mainThread()).subscribe(new Consumer<Long>() {
            @Override
