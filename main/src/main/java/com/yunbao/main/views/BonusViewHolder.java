...
public class BonusViewHolder extends AbsViewHolder implements View.OnClickListener {
    ...
    public void setData(List<BonusBean> list, int day, String continueDay, boolean hasSign) {
        mList = list;
        mDay = day;
        ...
        if (startIndex >= 0) {
            spannableString.setSpan(new ForegroundColorSpan(0xff7f5fff), startIndex, startIndex + continueDay.length(),
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        mTotalDay.setText(spannableString);
        String s1 = WordUtil.getString(R.string.bonus_sign_3);
//        int hasDay = hasSign ? mDay : mDay - 1;
        int hasDay = mDay;
        for (int i = 0; i < 7; i++) {
        ...
        if (hasSign) {
            ...
            mBtnSign.setText(R.string.bonus_sign_4);
        }
