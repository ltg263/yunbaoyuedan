...
//        RxRefreshView.ReclyViewSetting.createLinearSetting(this,1).settingRecyclerView(reclyView);
        reclyView.setHasFixedSize(true);
        reclyView.setLayoutManager(new LinearLayoutManager(mContext,LinearLayoutManager.VERTICAL,false));
    ...
    private void getData() {
        MainHttpUtil.getUserReport().compose(this.<List<CheckEntity>>bindToLifecycle()).subscribe(new DefaultObserver<List<CheckEntity>>() {
            @Override
    ...
    private void checkExitImgs(){
        ...
        if (uploadBeanList.isEmpty()){
            ...
            showLoadingDialog();
            getUploadStrategy(false,uploadBeanList);
        }
    }
    ...
    private void commit(String thumbs) {
        ...
        MainHttpUtil.setUserReport(mUid,content,thumbs).compose(this.<Boolean>bindToLifecycle()).subscribe(new DialogObserver<Boolean>(this) {
            @Override
    ...
    private void getUploadStrategy(final boolean isNeedCompass, final List<UploadBean> uploadBeanList){
        FileUploadManager.getInstance().createUploadImpl(mContext, new CommonCallback<UploadStrategy>() {
            ...
            public void callback(UploadStrategy strategy) {
                ...
                upLoadResource(isNeedCompass,uploadBeanList);
            }
        });
    }

    //上传网络资源
    private void upLoadResource(boolean isNeedCompass, List<UploadBean> uploadBeanList) {
