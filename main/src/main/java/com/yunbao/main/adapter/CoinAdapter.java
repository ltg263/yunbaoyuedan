...
//            mGiveGroup = itemView.findViewById(R.id.give_group);
        ...
        void setData(CoinBean bean, int position, Object payload) {
            if (payload == null) {
                ...
                if (Constants.PAY_TYPE_PAYPAL.equals(mCurrencyType)){
                    mCoin.setText(bean.getPaypalCoin());
                }else {
                    mCoin.setText(bean.getCoin());
                }
                mMoney.setText(StringUtil.contact(mMoneySymbol, bean.getMoney()));
...
//                }
    ...
    public void updatePayType(String type){
        if (type.equals(mCurrencyType)){
            return;
        }
        mCurrencyType = type;
        if (Constants.PAY_TYPE_PAYPAL.equals(type)){
