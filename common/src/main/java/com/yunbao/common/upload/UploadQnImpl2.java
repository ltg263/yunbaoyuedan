...
public class UploadQnImpl2 implements UploadStrategy {
...
//    private String mPrefix;
    ...
    public UploadQnImpl2(String uploadToken, String zone, String prefix) {
        mToken = DecryptUtil.decrypt(uploadToken);
        mZone = zone;
//        mPrefix = prefix + "_";
    ...
    private void uploadNext() {
        ...
        if (bean.getType() == UploadBean.IMG && mNeedCompress) {
            if (mLubanBuilder == null) {
                mLubanBuilder = Luban.with(CommonAppContext.sInstance)
                        .ignoreBy(8)//8k以下不压缩
    ...
    private void upload(UploadBean bean) {
        if (bean != null && !TextUtils.isEmpty(mToken) && mCompletionHandler != null) {
            if (mUploadManager == null) {
                //存储区域
                Zone zone = null;
                if (UploadInfoBean.UPLOAD_QI_NIU_HD.equals(mZone)) {//华东
                    zone = FixedZone.zone0;
                } else if (UploadInfoBean.UPLOAD_QI_NIU_HB.equals(mZone)) {//华北
                    zone = FixedZone.zone1;
                } else if (UploadInfoBean.UPLOAD_QI_NIU_HN.equals(mZone)) {//华南
                    zone = FixedZone.zone2;
                } else if (UploadInfoBean.UPLOAD_QI_NIU_BM.equals(mZone)) {//北美
                    zone = FixedZone.zoneNa0;
                } else if (UploadInfoBean.UPLOAD_QI_NIU_XJP.equals(mZone)) {//东南亚
